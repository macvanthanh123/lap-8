...
def chunk_documents(documents: list[dict]) -> list[dict]:
    ...
    try:
        ...
        print("langchain-text-splitters not installed. Using simple string slicing.")
        chunks = []
        for doc in documents:
            content = doc["content"]
            # Fallback naive chunking
            for i in range(0, max(1, len(content) - CHUNK_OVERLAP), CHUNK_SIZE - CHUNK_OVERLAP):
                chunk_text = content[i:i+CHUNK_SIZE]
                chunks.append({
                    "content": chunk_text,
                    "metadata": {**doc["metadata"], "chunk_index": i // (CHUNK_SIZE - CHUNK_OVERLAP)}
                })
        # Need to ensure at least one chunk per doc even if content is empty
        for doc in documents:
            if not doc["content"]:
                chunks.append({
                    "content": "",
    ...
    try:
        ...
        collection = client.collections.create(
            ...
            properties=[
                ...
                Property(name="doc_type", data_type=DataTyCreatedpe.TEXT),
            ]
        )
        ...
        print("Indexed chunks to Weaviate successfully.")
    except Exception as e:
        print(f"Failed to connect or index to Weaviate: {e}")
        print("Skipping actual indexing for now.")
...
def run_pipeline():
    ...
    with open("chunks_with_embedding.json", "w", encoding="utf-8") as f:
        json.dump(chunks, f, ensure_ascii=False, indent=2)
    print(f"✓ Embedded {len(chunks)} chunks")

    index_to_vectorstore(chunks)
    with open("chunks_indexed.json", "w", encoding="utf-8") as f:
        json.dump(chunks, f, ensure_ascii=False, indent=2)
